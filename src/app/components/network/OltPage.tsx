...
export function OltPage({ onNavigate }: OltPageProps) {
  ...
  const filteredOnus = useMemo(() => {
    const rawQ = onuSearch.trim().toLowerCase();
    if (!rawQ) return onuList;
    ...
    return onuList.filter(o => {
      ...
      const cleanOlt = o.oltServer.toLowerCase().replace(/[^a-z0-9]/g, '');
      const cleanId = (o.customerId || "").toLowerCase().replace(/[^a-z0-9]/g, '');

      return (
        o.mac.toLowerCase().includes(rawQ) ||
        cleanMac.includes(cleanQ) ||
        o.customer.toLowerCase().includes(rawQ) ||
        cleanCust.includes(cleanQ) ||
        o.ponPort.toLowerCase().includes(rawQ) ||
        cleanPon.includes(cleanQ) ||
        o.oltServer.toLowerCase().includes(rawQ) ||
        cleanOlt.includes(cleanQ) ||
        (o.customerId && o.customerId.toLowerCase().includes(rawQ)) ||
